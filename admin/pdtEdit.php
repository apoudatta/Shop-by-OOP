...
<?php
    if (!isset($_GET['pdtId']) || $_GET['pdtId'] == NULL) {
        echo "<script>window.location = 'productlist.php'</script>";
    }else{
        $id = preg_replace('/[^-a-zA-Z0-9_]/', '', $_GET['pdtId']);
    }

    $product = new Product();
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
        $productUpdate = $product->productUpdate($_POST, $_FILES, $id);
    }
?>
...
<div class="grid_10">
    ...
    ?>     
    <?php
        $getPdt = $product->getPdtById($id);
        if ($getPdt) {
            while ($value = $getPdt->fetch_assoc()) {
    ?>    
         <form action="" method="post" enctype="multipart/form-data">
            <table class="form">
                ...
                <tr>
                    ...
                    <td>
                        <img src="<?php echo $value['image'] ?>" height="120px" width="180px"><br>
